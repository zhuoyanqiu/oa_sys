layui.config({
	...
	//初始化顶级菜单
	$.getJSON(path + '/sysMenu/getMenuTop?t=' + Math.random(), function(menuData) {
		...
		$menu.find('li.layui-nav-item').each(function() {
			...
			$this.on('click', function() {
				...
				//---------这是第一个栗子----------
				$.getJSON(path + '/sysMenu/getMenuByPidRec',{id:id},function(data){
					navbar.set({
						elem: '#side',
						data: data
					});
		...
		//模拟点击内容管理
		$.getJSON(path + '/sysMenu/getTopMenuFirst?t=' + Math.random(), function(menuData) {
